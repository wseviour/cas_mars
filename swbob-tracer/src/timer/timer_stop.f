...
c*********************************************************************
      ...
      implicit none
c
c stop timer timer_id and return the number of seconds that have
c elapsed since the last start with timer_start
c
c**********************************************************************

      integer timer_id

      external timer_clock
      real*8 timer_clock
...
c**********************************************************************
c
c fetch the current clock
c
      timer_cur_time(timer_id) = timer_clock() -
     1 timer_cur_time(timer_id)
...
c
     ...
     1 (timer_assign(timer_id).eq.0) ) then
         write(6,*)"TIMER_STOP: Invalid timer id or timer not initialized"
         stop
      endif

c**********************************************************************
c
c set the accumulated time for this timer
c
      timer_accum_time(timer_id) = timer_accum_time(timer_id)+
     1 timer_cur_time(timer_id)
