...
c
     ...
     $ (timer_assign(timer_id).eq.0) ) then
         write(6,*)"TIMER_START: Invalid timer id or timer not initialized"
         stop
      endif

c**********************************************************************
c increment the number of starts
c
      timer_starts(timer_id)=timer_starts(timer_id) + 1

c**********************************************************************
c
c fetch the current time
c
